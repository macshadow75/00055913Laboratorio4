...
void createLocalP(Matrix &P,mesh m){
    ...
    float K = m.getParameter(UNKNOW_K);
...
void createLocalQ(Matrix &Q,mesh m){
    float l = m.getParameter(ELEMENT_LENGTH);
    float U = m.getParameter(UNKNOW_U);
...
Vector createLocalb(int element,mesh &m){
    ...
    float W = m.getParameter(UNKNOW_W), N = m.getParameter(UNKNOW_N), l = m.getParameter(ELEMENT_LENGTH);
